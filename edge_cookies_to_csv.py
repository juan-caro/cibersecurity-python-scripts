...
def get_edge_master_key(local_state_path):
    ...
    encrypted_key = encrypted_key[5:]  # quitar "DPAPI"
    master_key = win32crypt.CryptUnprotectData(encrypted_key, None, None, None, 0)[1]
    return master_key

def decrypt_edge_value(encrypted_value, master_key):
    try:
        ...
        elif encrypted_value[:3] in (b'v11', b'v20'):
...
def export_edge_cookies(profile_name, output_file):
    # Detectar rutas posibles
    edge_user_data = os.path.expandvars(r"%LOCALAPPDATA%\Microsoft\Edge\User Data")
    edge_legacy_data = os.path.expandvars(r"%LOCALAPPDATA%\MicrosoftEdge")

    # Usar Chromium Edge si existe
    if os.path.exists(edge_user_data):
        base_path = edge_user_data
    elif os.path.exists(edge_legacy_data):
        base_path = edge_legacy_data
    else:
        print("[!] No se encontró ninguna instalación de Edge.")
        return
    ...
    if not os.path.exists(local_state_path):
        print("[!] No se encontró el archivo Local State de Edge.")
        return
    ...
    temp_db = os.path.join(os.environ['TEMP'], f"Edge_{profile_name}_Cookies_temp.db")
...
if __name__ == "__main__":
    perfil = input("Introduce el nombre del perfil de Edge (ej: Default, Profile 1, Profile 2): ").strip()
    output_csv = f"edge_cookies_{perfil}.csv"
    export_edge_cookies(perfil, output_csv)
